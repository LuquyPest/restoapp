...
interface Props { employeeId: string; viewerUserId: string }

const POLL_INTERVAL = 4000

export default function EmployeeMessagesTab({ employeeId, viewerUserId }: Props) {
  ...
  const fetchMessages = useCallback(async (silent = false) => {
    const res = await fetch(`/api/employees/${employeeId}/messages`)
  ...
  useEffect(() => {
    fetchMessages()
    fetch(`/api/employees/${employeeId}/messages/read`, { method: "PATCH" })
    const id = setInterval(() => fetchMessages(true), POLL_INTERVAL)
    return () => clearInterval(id)
  }, [employeeId, fetchMessages])
  ...
  return (
    <div className="flex flex-col rounded-lg border h-[520px]">
