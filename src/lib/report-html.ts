...
<div class="bar-chart">${(data.dailyData ?? []).map((d: any) => {
  ...
  return `<div class="bar-wrap"><div class="bar" style="height:${h}px" title="${esc(f(d.value))}"></div><div class="bar-label">${esc(d.label)}</div></div>`
}).join("")}</div>
<table><thead><tr><th>Jour</th><th>Date</th><th>CA</th></tr></thead><tbody>
${(data.dailyData ?? []).map((d: any) => `<tr><td>${esc(d.label)}</td><td>${esc(d.date)}</td><td><strong>${f(d.value)}</strong></td></tr>`).join("")}
</tbody></table>
</div>
...
<div class="card">
  ...
  ${(data.charges ?? []).filter((ch: any) => ch.type === "DEDUCTIBLE").map((ch: any) => `<div class="bilan-row indent"><span>${esc(ch.name)}</span><span>${f(ch.amount)}</span></div>`).join("")}
  <div class="bilan-row bold" style="margin-top:8px"><span>Total non déductibles</span><span>${f(data.chargesNonDeductible)}</span></div>
  ${(data.charges ?? []).filter((ch: any) => ch.type === "NON_DEDUCTIBLE").map((ch: any) => `<div class="bilan-row indent"><span>${esc(ch.name)}</span><span>${f(ch.amount)}</span></div>`).join("")}
</div>
<div class="card">
  ...
  ${(data.partnerSummary ?? []).map((p: any) => `<div class="bilan-row indent"><span>${esc(p.name)}</span><span>${f(p.revenue)}</span></div>`).join("")}
</div>
</div>
</div>
...
<tr>
  ...
  <td>${esc(e.grade)}</td>
  <td class="mono">${esc(e.accountNumber ?? "—")}</td>
  <td>${f(e.revenue)}</td>
...
${(data.loyaltyCards ?? []).map((lc: any) => {
  ...
  return `<tr>
    <td><strong>${esc(lc.name)}</strong></td>
    <td>${lc.discount}%</td>
...
${(data.suppliers ?? []).map((s: any) => `<tr><td><strong>${esc(s.name)}</strong></td><td>${esc(s.contact ?? "—")}</td><td>${esc(s.email ?? "—")}</td><td>${esc(s.phone ?? "—")}</td></tr>`).join("")}
</tbody></table>
...
${(data.allOrders ?? []).slice(0, 100).map((o: any) => `<tr>
  ...
  <td style="font-size:11px;max-width:200px">${o.lines.map((l: any) => `${esc(l.qty)}× ${esc(l.name)}`).join(", ")}</td>
  <td>${esc(o.partnerName ?? o.loyaltyName ?? "—")}</td>
